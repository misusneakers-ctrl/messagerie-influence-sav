...
async function getProductDetail(shopDomain, handle, opts = {}) {
  ...
  if (token) {
    try {
      ...
      // Droits insuffisants, API en panne… : on tente la vitrine.
    }
  }
 ...
 * Lève une erreur code 'boutique_inaccessible' si rien n'est joignable.
 */
async function searchProducts({ shopDomain, tenant, accessToken, query, limit = 5 }) {
  ...
  if (token) {
    try {
      ...
      // Zéro résultat côté Admin : la vitrine indexe parfois autrement
      // (synonymes, tags). On lui laisse sa chance avant de renoncer.
    } catch {
      // On bascule sur la vitrine.
    }
  }
...
async function checkStock({ shopDomain, tenant, accessToken, query, size }) {
  ...
  let found;
  try {
    found = await searchProducts({ shopDomain: domain, tenant, accessToken, query: q, limit: 5 });
  } catch (err) {
