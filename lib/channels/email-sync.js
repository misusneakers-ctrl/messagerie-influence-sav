...
function parseGmailMessage(msg, mailbox) {
  ...
  // Correctif 2026-09-18 : on conserve `attachment_id` et `inline`, sans quoi
  // le contenu réel de la pièce jointe ne peut pas être téléchargé ensuite
  // (voir lib/channels/email-attachments.js).
  const attachments = listAttachments(msg.payload).map((a) => ({
    filename: a.filename, mime_type: a.mime_type, size: a.size,
    attachment_id: a.attachment_id || null, inline: !!a.inline,
  }));
...
async function syncEmailInbox(tenant, { maxMessages = 40, timeBudgetMs = 45000 } = {}) {
  ...
  const summary = { mailbox, query, scanned: 0, tickets_created: 0, messages_created: 0, attachments_stored: 0, attachments_backfilled: 0, skipped: {}, remaining: 0, errors: [] };
      ...
      await withTenant(tenant.id, async (client) => {
        ...
        const insertInto = async (ticket, list, contactHint) => {
          ...
          for (const m of list) {
            ...
            // Correctif 2026-09-18 : contenu réel des pièces jointes, déposé
            // sur Blob puis affiché dans le fil comme pour Instagram.
            let files = { attachments: [], skipped: [] };
            if (m.attachments.length) {
              try {
                files = await fetchEmailAttachments(accessToken, tenant.slug, m, { deadline: started + timeBudgetMs });
              } catch (err) {
                summary.errors.push({ gmail_message_id: m.id, error: 'pieces_jointes: ' + err.message });
              }
              summary.attachments_stored += files.attachments.length;
            }
            await client.query(
              ...
              [tenant.id, ticket.id, m.outbound ? 'outbound' : 'inbound', m.outbound ? 'sent' : 'received',
                (m.text || '(e-mail sans texte)') + skippedNote(files.skipped), m.id, m.internalDate.toISOString(),
                JSON.stringify(files.attachments), JSON.stringify(m.emailMeta)]
            );
            inserted += 1;
          ...
          // Ajout 2026-09-18 : même règle que sur Instagram — un numéro de
          // commande écrit par la cliente est rattaché au ticket tout de
          // suite, après vérification chez Shopify. Objet compris : beaucoup
          // de clientes répondent à leur e-mail de confirmation, qui porte le
          // numéro dans son sujet.
          try {
            const texte = list.filter((m) => !m.outbound)
              .map((m) => [m.subject, m.text].filter(Boolean).join('\n')).join('\n');
            const lien = await autoLinkOrder(client, tenant, ticket, texte);
            if (lien) summary.orders_linked = (summary.orders_linked || 0) + 1;
          } catch (err) {
            // Surtout PAS summary.errors : une erreur là-dedans fige le
            // curseur Gmail et bloque la synchro (leçon du 17/09 avec le
            // rattrapage des pièces jointes). Un rattachement raté n'est pas
            // une raison de ne plus recevoir les e-mails.
            (summary.order_link_errors = summary.order_link_errors || [])
              .push({ ticket_id: ticket.id, error: String(err.message || err).slice(0, 200) });
          }
        };
  ...
  // Rattrapage des e-mails importés AVANT le 18/09 : ils n'ont que le nom de
  // leurs pièces jointes, pas le contenu. On en reprend quelques-uns à chaque
  // « Actualiser », dans le temps qui reste, des plus récents aux plus
  // anciens — aucun appel supplémentaire quand tout est à jour.
  if (Date.now() - started < timeBudgetMs * 0.7) {
    try {
      const done = await backfillEmailAttachments(tenant, {
        accessToken, mailbox, limit: 10, deadline: started + timeBudgetMs,
      });
      summary.attachments_backfilled = done.attachments;
      // Les soucis de rattrapage sont rapportés à part : ils ne doivent pas
      // bloquer l'avancée du curseur de synchronisation (sinon chaque
      // « Actualiser » rescannerait 14 jours indéfiniment).
      if (done.errors.length) summary.backfill_errors = done.errors;
    } catch (err) {
      summary.backfill_errors = [{ error: 'rattrapage_pieces_jointes: ' + err.message }];
    }
  }
...
/**
 * Rattrapage : e-mails déjà en base dont les pièces jointes n'ont jamais été
 * téléchargées (import antérieur au 18/09). On relit le message chez Gmail
 * pour retrouver les identifiants de pièces jointes, on télécharge, et on
 * complète la ligne. Rien n'est réimporté ni dupliqué.
 * Retourne { messages, attachments, errors }.
 */
async function backfillEmailAttachments(tenant, { accessToken, mailbox, limit = 10, deadline = Infinity } = {}) {
  const out = { messages: 0, attachments: 0, errors: [] };
  const rows = await withTenant(tenant.id, async (client) => {
    const { rows } = await client.query(
      `SELECT id, external_message_id, body
         FROM ticket_messages
        WHERE tenant_id = $1 AND channel = 'email'
          AND external_message_id IS NOT NULL
          AND COALESCE(jsonb_array_length(attachments), 0) = 0
          AND COALESCE(jsonb_array_length(email_meta -> 'attachments'), 0) > 0
  ...
  for (const row of rows) {
    ...
    try {
      const full = parseGmailMessage(await gmailFetch(accessToken, `/messages/${row.external_message_id}?format=full`), mailbox);
      const files = await fetchEmailAttachments(accessToken, tenant.slug, full, { deadline });
      if (!files.attachments.length && !files.skipped.length) continue;
      await withTenant(tenant.id, async (client) => {
        await client.query(
          ...
          [JSON.stringify(files.attachments), skippedNote(files.skipped), row.id, tenant.id]
        );
      });
      out.messages += 1;
      out.attachments += files.attachments.length;
    } catch (err) {
      out.errors.push({ gmail_message_id: row.external_message_id, error: 'rattrapage: ' + err.message });
    }
  }
  return out;
}
...
module.exports = {
  syncEmailInbox, backfillEmailAttachments, isAutomated, parseGmailMessage,
  threadKey, correspondentOf, recipientsOf, isNotificationSender,
};
