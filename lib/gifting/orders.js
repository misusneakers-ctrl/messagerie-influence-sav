...
async function buildGiftingProposal(tenant, ticketId) {
  ...
  try {
    if (item && item.product_handle) {
      const detail = await getProductDetail(shop, item.product_handle, { tenant });
      candidates = [detail];
    }
    if (candidates.length === 0 && item && (item.model || item.color)) {
      candidates = await searchProducts({ shopDomain: shop, tenant, query: [item.model, item.color].filter(Boolean).join(' '), limit: 5 });
    }
  } catch (err) {
...
async function createGiftingOrder(tenant, ticketId, body, actor) {
  ...
  let product;
  try {
    product = await getProductDetail(tenant.myshopify_domain, String(b.product_handle), { tenant });
  } catch (err) {
