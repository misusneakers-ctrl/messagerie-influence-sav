...
const { repairMojibake } = require('../text/clean');
...
function listAttachments(payload, out = []) {
  ...
  if (payload.filename) {
    // Ajout 2026-09-18 : on retient aussi si la pièce est « inline » (image
    // insérée dans le corps ou logo de signature, repérée par son Content-ID
    // ou un Content-Disposition: inline) — elles sont téléchargées mais
    // filtrées quand elles sont minuscules, pour ne pas polluer le fil avec
    // les logos de signature.
    const headers = payload.headers || [];
    const disposition = String(header(headers, 'Content-Disposition') || '').toLowerCase();
    out.push({
      filename: payload.filename,
      mime_type: payload.mimeType,
      size: payload.body && payload.body.size,
      attachment_id: payload.body && payload.body.attachmentId,
      inline: disposition.startsWith('inline') || !!header(headers, 'Content-ID'),
    });
  }
...
function extractText(payload) {
  ...
  // Correctif 2026-09-18 : certains clients mail (Outlook Android surtout)
  // annoncent iso-8859-1 alors qu'ils envoient de l'UTF-8 — le message
  // arrivait avec « je me suis trompÃ©e ». On répare à la lecture, pour que
  // ce qui est ENREGISTRÉ soit déjà propre.
