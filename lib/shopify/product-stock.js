...
// Deux niveaux de détail. Le niveau COMPLET donne le stock par emplacement
// (MOBILTRON / SHOWROOM TURBIGO) ; s'il est refusé — un droit qui manque, un
// champ retiré d'une version d'API — on retombe sur le niveau SIMPLE, qui
// suffit à répondre « disponible ou non ». Mieux vaut perdre le détail par
// emplacement que retomber sur la vitrine publique, qui ment.
const VARIANT_CORE = `
      id
      title
      sku
...
const productFields = (withLocations) => `
  id
  title
  handle
  status
  ...
  variants(first: 50) {
    nodes {${VARIANT_CORE}${withLocations ? VARIANT_LOCATIONS : ''}
    }
  }`;

const searchQuery = (withLocations) => `
  query StockSearch($q: String!, $n: Int!) {
    products(first: $n, query: $q) { nodes { ${productFields(withLocations)} } }
  }`;

const byHandleQuery = (withLocations) => `
  query StockByHandle($handle: String!) {
    productByIdentifier(identifier: { handle: $handle }) { ${productFields(withLocations)} }
  }`;
...
/**
 * Exécute une requête en essayant d'abord le détail par emplacement, puis
 * sans. Si les deux échouent, l'erreur remonte : elle sera tracée, jamais
 * avalée — c'est ce silence qui a laissé passer le bug du 17-18/09.
 */
async function withDegradation(build, { shopDomain, accessToken, variables }) {
  let lastErr = null;
  for (const withLocations of [true, false]) {
    try {
      return await adminGraphql({ shopDomain, accessToken, query: build(withLocations), variables });
    } catch (err) {
      lastErr = err;
    }
  }
 ...
 * le libellé exact du catalogue. Dernier essai : la recherche brute, sans
 * opérateur, au cas où la syntaxe à étoiles ne conviendrait pas.
 */
async function adminSearchProducts({ shopDomain, accessToken, query, limit = 5 }) {
  const terms = buildSearchQuery(query);
  if (!terms.length) return [];
  const tentatives = [];
  for (let keep = terms.length; keep >= 1; keep -= 1) tentatives.push(terms.slice(0, keep).join(' AND '));
  tentatives.push(normalizeTerm(query)); // recherche brute
  for (const q of tentatives) {
    const data = await withDegradation(searchQuery, { shopDomain, accessToken, variables: { q, n: limit } });
...
async function adminGetProductByHandle({ shopDomain, accessToken, handle }) {
  const data = await withDegradation(byHandleQuery, {
    shopDomain, accessToken, variables: { handle: String(handle) },
  });
